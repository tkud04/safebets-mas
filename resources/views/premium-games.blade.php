    <section id="premium-games">
	  <center><h2 class="section-heading">Premium Games</h2></center>
	  <hr>
      <div class="container">
        <div class="row">
		  @if(isset($premiumGames) && count($premiumGames) > 0)
		  @foreach($premiumGames as $p)
		  <?php 
		    $category = $t["category"];
		    $type = $t["type"];
          ...
          <div class="col-lg-3col-md-12 ml-auto text-center">
                        <div class="card mt-5">
                            <div class="card-header"><span class="badge {{$badgeClass}}">{{$p["category"]}}</span></div>
                            <div class="card-body">
                                <h4 class="card-title">{{$p["desc"]}}</h4>
								<hr class="hr-success">
                                <h6 class="card-subtitle">{{$p["type"]}}</h6>
								<p>{{$p["league"]}}</p>
								<hr class="hr-success">
								<a href="#" class="btn btn-success">{{$cta}}</a>
                            </div>
							<div class="card-footer text-muted">{{$p["date"]}}</div>
                        </div>
          </div>
		  @endforeach
		  @endif
